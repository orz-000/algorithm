...
int main(){
  ...
  for(i=0;S[i]!='\0';i++){
    /* [ Complete Here!! ] */;  // Count the appearance of each character
    index[(int)S[i]]++;
  }
  ...
  while(i>1){
    ...
    printf("minheap %c,%d,%c,%d\n",couple[0].word,couple[0].count,couple[1].word,couple[1].count);
...
void downheap(Node *A, int k, int n){
  ...
  while (k<=n/2) {
    ...
    if (j<n){
      if(A[j+1].count < A[j].count /* [ Complete Here!! ] */ || (A[j].count == A[j+1].count && A[j].word > A[j+1].word) ){ //ヒープ条件の確認（Ex02とほぼ同じ）
        j++;
      }
    }

    if (v.count < A[j].count /* [ Complete Here!! ] */ || (v.count == A[j].count && v.word > A[j].word) ) break;
...
Node delete_min(Node *A,int *i){
  ...
  A[1] = A[n];
  construct_2(A,n);/* [ Complete Here!! (Write function call) ] */; //ヒープに戻す（Ex02とほぼ同じ）
...
int Huffman_DFS(NodePointer p,char *bin){
  ...
  while(1){
    ...
    if(p->c_left != NULL && p->c_left->vertex.visited == 0/* [ Complete Here!! ] */){ //DFSの要領で二分木を左側優先でたどる（もし、左側の子が探索可能で、かつ未到達であるならば。 Ex03とほぼ同じ）
      strcat(bin,"1");
      i++;
      totalbits = Huffman_DFS(p->c_left,bin);
    }
    else if(p->c_right != NULL && p->c_right->vertex.visited == 0/* [ Complete Here!! ] */){ //DFSの要領で二分木を右側にたどる（もし、右側の子が探索可能で、かつ未到達であるならば。 Ex03とほぼ同じ）
      strcat(bin,"0");
      i++;
